...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << basename(argv[0])
                  << " #_of_layers unit_length" << std::endl;
        std::cerr << "If unit_length<=0, only the total particle number "
                  << "will be reported." << std::endl;
        return -1;
    }
    ...
    if (n < 0) {
        std::cerr << "n=" << n << ": #_of_layers must >=0." << std::endl;
        return -2;
    }
    ...
    for (int i = 0; i < num; i++)
        std::cout << p[i].s << " " << p[i].v * d << std::endl;
